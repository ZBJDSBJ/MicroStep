...
public class generatePedometerDataBase {
    ...
    public static void main(String[] args) throws Exception {
        Schema schema = new Schema(1, "com.alost.microstep.data.model.database.core");
        schema.enableKeepSectionsByDefault();

        addPedometerCard(schema);
        addPedometerDailyDetail(schema);
        addCalendarCard(schema);

        new DaoGenerator().generateAll(schema, "/Users/700sfriend/zou/workpalce/MicroStep/app/src/main/java/");
    ...
    private static void addPedometerCard(Schema schema) {
        ...
        PedometerCard.setTableName("PedometerCard");
        //默认的id自主增加
        ...
        //计步算法优化,存协处理器的值:核心算法
        PedometerCard.addFloatProperty("stepCounterValue");
    }

    //每日走路的详情表
    private static void addPedometerDailyDetail(Schema schema) {
        Entity dailyDetail = schema.addEntity("PedometerDailyDetailEntity");
        dailyDetail.setTableName("PedometerDailyDetail");

        dailyDetail.addLongProperty("id");
        dailyDetail.addIntProperty("stepCount");
        dailyDetail.addLongProperty("startTime");
        dailyDetail.addLongProperty("endTime");
        dailyDetail.addLongProperty("delTime");
